...
def generate_entries(firstnames: Path, lastnames: Path, rockyou: Path):
	...
	for i in range(NUM_ENTRIES):
		...
		pw_num_part = "".join([random.choice(digits) for _ in range(random.randint(1, 4))])
		pw_special_part = "".join([random.choice(punctuation) for _ in range(random.randint(1, 4))])
...
def generate_entries_and_write_dataset(firstnames: Path, lastnames: Path, rockyou: Path, dest: Path):
...
def main():
	if len(sys.argv) != 5:
		print(f"Usage: python create_password_dataset.py <firstnames> <lastnames> <rockyou> <destination-dir>")
		sys.exit()
