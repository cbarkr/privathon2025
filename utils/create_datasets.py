...
def generate_entries_and_write_datasets(
    firstnames: Path, lastnames: Path, cities: Path, dest: Path
):
...
def main():
    if len(sys.argv) != 5:
        print(
            f"Usage: python create_datasets.py <firstnames> <lastnames> <cities> <destination dir>"
        )
        sys.exit()
